...
object DeviceController {
  ...
  suspend fun getCode(id: Long?, merchantId: Long?): DeviceModel? {
    val query =
      "select code, action, exp_action from device where id = $id and merchant_id = $merchantId"
      ...
      DBManager.connection().use {
        val rs =
          it
            ...
            .apply {
              var x = 0
              if (device?.action != null) this.setString(++x, device.action)
              if (device?.phone != null) this.setString(++x, device.phone)
              this.closeOnCompletion()
            }
            .executeQuery()
        if (rs.next()) rs.getLong("code") else null
      }
    }
  }
    ...
    return withContext(DBManager.databaseDispatcher) {
      DBManager.connection().use { connection ->
        ...
        return@withContext device.copy(
          token = JwtConfig.generateDeviceToken(uuid = device.uuid, merchantId = device.merchantId)
        )
      }
    }
  }
  ...
  suspend fun boardAuth(device: BoardDeviceModel): BoardDeviceModel {
    ...
    return withContext(DBManager.databaseDispatcher) {
      ...
      DBManager.connection().use { connection ->
        ...
        return@withContext device.copy(
          token =
            JwtConfig.generateBoardDeviceToken(
              uuid = device.uuid,
