        ...
        if (checkCode != Status.ACCEPTED) {
          ...
          if (oldCode != device.code?.toLongOrNull()) {
            ...
            if (stuff != null) {
              ...
              device.token =
                JwtConfig.generateStaffToken(merchantId = principal?.merchantId, uuid = sessionUuid)
        ...
        if (result.isOk()) {
          ...
          call.respond(
            staff.copy(
              ...
              token = JwtConfig.generateStaffToken(merchantId = pr?.merchantId, uuid = sessionUuid)
            )
          )
        } else {
          call.respond(result)
