...
object FlatService : FlatRepository {
  ...
  override suspend fun getAll(): List<FlatTable?> =
    DBManager.getData(dataClass = FlatTable::class, tableName = FLAT_TABLE_NAME)
      .filterIsInstance<FlatTable?>()

  override suspend fun get(id: Long?): FlatTable? =
    DBManager.getData(dataClass = FlatTable::class, id = id, tableName = FLAT_TABLE_NAME)
      .firstOrNull() as FlatTable?
