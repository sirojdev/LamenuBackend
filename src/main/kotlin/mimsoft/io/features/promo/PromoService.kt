...
object PromoService {
  ...
  suspend fun update(dto: PromoDto): Boolean {
    ...
    withContext(DBManager.databaseDispatcher) {
      repository.connection().use {
        rs =
          it.prepareStatement(query.toString()).use { ti ->
  ...
  suspend fun delete(id: Long?, merchantId: Long?): Boolean {
    ...
    withContext(DBManager.databaseDispatcher) {
      repository.connection().use {
        rs =
          it.prepareStatement(query.toString()).apply { this.closeOnCompletion() }.executeUpdate()
      }
    }
    return rs == 1
  }
  ...
  suspend fun get(merchantId: Long?, id: Long?): PromoDto? {
    val query =
      "select * from $PROMO_TABLE_NAME where merchant_id = $merchantId and id = $id and deleted = false"
