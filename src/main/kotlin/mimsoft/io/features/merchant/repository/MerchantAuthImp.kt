...
object MerchantAuthImp : MerchantAuthService {
  ...
  override suspend fun auth(merchantDto: MerchantDto?): MerchantDto? {
    ...
    sessionService.auth(SessionTable(merchantId = merchant.id, uuid = uuid))
    return merchant.copy(
      token = JwtConfig.generateMerchantToken(merchantId = merchant.id, uuid = uuid)
    )
  }
  ...
  override suspend fun getByPhonePassword(phone: String?, password: String?): MerchantDto? {
    ...
    val query =
      "select id, phone  from merchant " + "where phone = ? and password = ? and not deleted"
