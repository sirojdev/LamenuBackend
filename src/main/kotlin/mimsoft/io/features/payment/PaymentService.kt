...
object PaymentService {
  ...
  suspend fun get(merchantId: Long?): PaymentDto? {
    val query =
      "select * from $PAYMENT_TABLE_NAME where merchant_id = $merchantId and deleted = false"
    return withContext(Dispatchers.IO) {
      repository.connection().use {
        val rs = it.prepareStatement(query).executeQuery()
        if (rs.next()) {
          return@withContext (PaymentDto(
  ...
  suspend fun add(paymentDto: PaymentDto): ResponseModel {
    ...
    return if (checkMerchant != null)
      ResponseModel(body = update(paymentDto = paymentDto), httpStatus = ResponseModel.OK)
    else {
      ResponseModel(
        body =
          (repository.postData(
