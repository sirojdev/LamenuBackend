...
fun Route.routeToLabel() {
  ...
  get("labels") {
    ...
    val labels =
      labelRepository.getAll(merchantId = merchantId, branchId = branchId).map {
        LabelMapper.toLabelDto(it)
      }
    if (labels.isEmpty()) {
  ...
  get("label/{id}") {
    ...
    val label =
      LabelMapper.toLabelDto(
        labelRepository.get(id = id, merchantId = merchantId, branchId = branchId)
      )
    if (label != null) {
      call.respond(label)
      return@get
  ...
  post("label") {
    ...
    val response =
      labelRepository.add(
        LabelMapper.toLabelTable(label.copy(merchantId = merchantId, branchId = branchId))
      )
...
data class LabelId(val id: Long? = null)
