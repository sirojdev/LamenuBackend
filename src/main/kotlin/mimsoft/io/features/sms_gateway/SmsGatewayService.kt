...
object SmsGatewayService {
  ...
  suspend fun get(merchantId: Long?): SmsGatewayDto? {
    val query =
      "select * from $SMS_GATEWAY_TABLE where merchant_id = $merchantId and deleted = false"
  ...
  suspend fun add(smsGatewayDto: SmsGatewayDto): ResponseModel {
    ...
    return if (checkMerchant != null)
      ResponseModel(body = update(smsGatewayDto = smsGatewayDto), httpStatus = ResponseModel.OK)
    else {
      ResponseModel(
        body =
          (repository.postData(
  ...
  suspend fun update(smsGatewayDto: SmsGatewayDto?): Boolean {
    return withContext(Dispatchers.IO) {
      val query =
        "update $SMS_GATEWAY_TABLE s set " +
