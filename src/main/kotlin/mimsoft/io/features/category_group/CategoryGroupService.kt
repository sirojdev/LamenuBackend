...
object CategoryGroupService {
  ...
  suspend fun delete(id: Long?, merchantId: Long?): Boolean {
    val query =
      "update $CATEGORY_GROUP_TABLE set deleted = true where id = $id and merchant_id = $merchantId"
  ...
  suspend fun getCategoryGroupWithBranchId(
    merchantId: Long?,
    branchId: Long?
...
order by cg.priority,
      ...
      repository.connection().use {
        ...
        if (rs.next()) {
          ...
          dtoList.map {
            ...
            it?.copy(products = list1)?.let { it1 -> categoryList.add(it1) }
          }
  ...
  suspend fun getCategoryGroupByIdInBranch(
    branchId: Long?,
    groupId: Long?
  ): MutableList<CategoryDto> {
    val query =
      """
    ...
    withContext(DBManager.databaseDispatcher) {
      repository.connection().use {
        val rs = it.prepareStatement(query).apply { closeOnCompletion() }.executeQuery()
        while (rs.next()) {
          list.add(
            CategoryDto(
              id = rs.getLong("id"),
