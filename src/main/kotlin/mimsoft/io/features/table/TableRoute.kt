...
fun Route.routeToTable() {
  ...
  get("tables") {
    ...
    val tables =
      tableService.getAll(merchantId = merchantId, branchId = branchId).map {
        tableMapper.toTableDto(it)
      }
    if (tables.isEmpty()) {
  ...
  get("tableByRoom") {
    ...
    val response =
      tableService.getTablesWaiter(roomId = roomId, merchantId = merchantId, branchId = branchId)
    if (response.isEmpty()) {
      call.respond(HttpStatusCode.NoContent)
      return@get
  ...
  get("getTable") {
    ...
    val response =
      TableService.getTableWithWaiter(id = tableId, branchId = branchId, merchantId = merchantId)
    if (response == null) {
      call.respond(HttpStatusCode.NoContent)
      return@get
    }
  ...
  get("table/{id}") {
    ...
    val table =
      TableMapper.toTableDto(
        tableService.get(id = id, merchantId = merchantId, branchId = branchId)
      )
    if (table == null) {
      call.respond(HttpStatusCode.NoContent)
      return@get
  ...
  put("table") {
    ...
    val table = call.receive<TableDto>()
    val response =
      tableService.update(table.copy(merchantId = merchantId, branch = BranchDto(id = branchId)))
    if (response) {
