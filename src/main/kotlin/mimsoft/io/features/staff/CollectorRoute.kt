...
fun Route.routeToCollector() {
  ...
  route("collector") {
    get("all") {
      ...
      val couriers =
        staffService.getAllCollector(merchantId = merchantId, branchId = branchId, limit, offset)
    ...
    get("/{id}") {
      ...
      val collector =
        staffService.getCollector(id = id, merchantId = merchantId, branchId = branchId)
      if (collector == null) {
        call.respond(HttpStatusCode.NoContent)
        return@get
      }
