...
object OrderUtils {
  ...
  fun query(params: Map<String, *>?, columnsSet: Set<String>): String {
    ...
    val joins =
      if (columnsSet.contains("order_price"))
    ...
    params?.let {
      ...
      if (params["status"] != null) {
        ...
        conditions +=
          " AND o.status IN (${(params["statuses"] as? List<*>)?.joinToString { ", " }}) "
      }
  ...
  fun searchQuery(params: Map<String, *>?, vararg columns: String): Search {
    ...
    var query =
      """
            ...
            o.branch_id o_branch_id 
        """
        .trimIndent() + (if (columnsSet.contains("products")) ", o.products o_products\n" else "")
    var joins =
      if (columnsSet.contains("order_price"))
        """
            FROM orders o
    ...
    joins +=
      (if (columnsSet.contains("user")) "LEFT JOIN users u ON o.user_id = u.id \n" else "") +
        ...
        (if (columnsSet.contains("payment_type"))
          "LEFT JOIN payment_type p ON o.payment_type = p.id \n"
        else "") +
    ...
    conditions +=
      " ORDER BY o.id DESC LIMIT ${params?.get("limit") as Int} OFFSET ${params["offset"] as Int} "
    val queryParams: MutableMap<Int, String> =
      mutableMapOf(
        //            1 to search,
        //            2 to search,
        //            3 to search,
  ...
  fun generateQuery(
    ...
    conditions?.let { conditionMap ->
      val conditionsList =
        conditionMap.entries.map { (tableName, columnValues) ->
  ...
  data class Search(val query: String, val queryParams: Map<Int, *>)
  ...
  fun parseGetAll(result: Map<String, *>, columns: Set<String>): Order {
    ...
    return Order(
      ...
      user =
        UserDto( // if (columns.contains("user"))
      ...
      paymentMethod =
        PaymentTypeDto(id = (result.getOrDefault("o_payment_type", null) as? Int?)?.toLong()),
  ...
  fun parse(result: Map<String, *>): Any {
    return Order(
      ...
      paymentMethod =
        PaymentTypeDto(id = (result.getOrDefault("payment_type", null) as? Int?)?.toLong()),
