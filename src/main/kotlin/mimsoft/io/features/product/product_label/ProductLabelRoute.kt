...
fun Route.routeToProductLabel() {
  ...
  route("product/label") {
    ...
    get("{id}") {
      ...
      val response =
        productLabelService.getLabelsByProductId(productId = id, merchantId = merchantId)
      if (response.isEmpty()) {
    ...
    delete {
      ...
      val response =
        productLabelService.deleteProductLabel(productLabelDto.copy(merchantId = merchantId))
      if (!response) {
        call.respond(HttpStatusCode.NoContent)
        return@delete
      }
