...
fun Route.routeToProductOption() {
  ...
  route("product/option") {
    ...
    get("{id}") {
      ...
      val response =
        productOptionService.getOptionsByProductId(productId = id, merchantId = merchantId)
      if (response.isEmpty()) {
    ...
    delete {
      ...
      val response =
        productOptionService.deleteProductOption(productOptionDto.copy(merchantId = merchantId))
      if (!response) {
        call.respond(HttpStatusCode.NoContent)
        return@delete
      }
