...
object ProductRepositoryImpl : ProductRepository {
  ...
  override suspend fun getAllProductInfo(
    merchantId: Long?,
    branchId: Long?
  ): List<ProductInfoDto?> {
    var query =
  ...
  override suspend fun get(id: Long?, merchantId: Long?): ProductTable? {
    ...
    val some =
      if (merchantId == null) {
        ...
        mapOf("merchant_id" to merchantId as Any, "id" to id as Any)
      }
    val data =
      repository
        .getPageData(dataClass = ProductTable::class, where = some, tableName = PRODUCT_TABLE_NAME)
        ?.data
        ?.firstOrNull()
    return data
  }
      ...
      repository.connection().use {
        ...
        if (rs.next()) {
          return@withContext ProductInfoDto(
            ...
            options =
              OptionRepositoryImpl.getOptionsByProductId(merchantId = merchantId, productId = id),
            extras =
              ExtraRepositoryImpl.getExtrasByProductId(merchantId = merchantId, productId = id)
          )
        } else return@withContext null
      }
    }
            ...
            ProductDto(
              ...
              options =
                GSON.fromJson(
                  ...
                  object : TypeToken<List<OptionDto>>() {}.type
                ),
            )
        }
      }
