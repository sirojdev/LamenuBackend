...
object BadgeService {
    ...
    suspend fun getAll(): List<BadgeDto?> {
        return repository.getData(dataClass = BadgeTable::class, tableName = BADGE_TABLE_NAME)
            .filterIsInstance<BadgeTable?>().map { BadgeMapper.toDto(it) }
    }
    ...
    suspend fun add(badge: BadgeDto?): Long? {
       return repository.postData(dataClass = BadgeTable::class, dataObject = BadgeMapper.toTable(badge), tableName = BADGE_TABLE_NAME)
    }

    suspend fun update(badge: BadgeDto?): Boolean {
        return repository.updateData(dataClass = BadgeTable::class, dataObject = BadgeMapper.toTable(badge), tableName = BADGE_TABLE_NAME)
    }
