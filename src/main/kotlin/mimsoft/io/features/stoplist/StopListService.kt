...
object StopListService {
  ...
  suspend fun decrementCount(id: Long?, merchantId: Long?): Boolean {
    ...
    withContext(DBManager.databaseDispatcher) {
      rs = repository.connection().use { it.prepareStatement(query).executeUpdate() }
    }
    return rs == 1
  }
  ...
  suspend fun add(stopListDto: StopListDto): ResponseModel {
    ...
    return if (checkStopList != null) {
      ResponseModel(body = updateCount(stopListDto = stopListDto), httpStatus = ResponseModel.OK)
    } else {
      ResponseModel(
        body =
          (repository.postData(
  ...
  suspend fun getByProduct(productId: Long?): StopListDto? {
    val query =
      "select * from $STOP_LIST_TABLE_NAME where product_id = $productId and deleted = false"
  ...
  suspend fun getAll(merchantId: Long?): List<StopListDto> {
    val query =
      "select * from $STOP_LIST_TABLE_NAME where merchant_id = $merchantId and deleted = false"
    return withContext(Dispatchers.IO) {
      repository.connection().use {
