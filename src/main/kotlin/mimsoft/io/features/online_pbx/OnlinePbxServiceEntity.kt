...
object OnlinePbxServiceEntity { // TODO: There is a drawback... Kamchilik bor...
  ...
  suspend fun add(onlinePbx: OnlinePbxEntity?): ResponseModel {
    get(onlinePbx?.domain) ?: return ResponseModel(httpStatus = ALREADY_EXISTS)
  ...
  suspend fun getAll(): List<OnlinePbxEntity?>? {
    return repository
      .getPageData(dataClass = OnlinePbxEntity::class, tableName = "online_pbx")
      ?.data
  }
  ...
  suspend fun delete(id: Long?): ResponseModel {
    return ResponseModel(
      body = repository.deleteData(tableName = "online_pbx", whereValue = id),
      httpStatus = ResponseModel.OK
    )
  }
}
