...
object YandexRepository {
  ...
  suspend fun getYandexOrder(orderId: Long?): YandexOrderDto? {
    val query = "select * from $YANDEX_ORDER where order_id = $orderId"
  ...
  suspend fun update(orderId: Long?, version: Int, status: String?): Boolean {
    val query =
      """update yandex set version = $version ,order_status = ? where order_id = $orderId"""
  ...
  suspend fun getYandexOrderList(
    branchId: Long?,
    limit: Int,
    offset: Int
  ): ArrayList<YandexOrderDto> {
  ...
  suspend fun updateCallBack(myOrderId: String?, claimId: String?, updatedTs: String?): Boolean {
    val query =
      """update yandex set updated_date = now()  where order_id = $myOrderId and claim_id = ?"""
