...
object IIkoService {
  ...
  suspend fun getOrganization(merchantId: Long): ResponseData {
    ...
    val response =
      client.post("https://api-ru.iiko.services/api/1/organizations") {
    ...
    return ResponseData(
      data = GSON.fromJson(response.body<String>(), IIkoOrganization::class.java).organizations
    )
  }
  ...
  suspend fun auth(merchantId: Long?): String {
    ...
    val response =
      client.post("https://api-ru.iiko.services/api/1/access_token") {
        contentType(ContentType.Application.Json)
        setBody(body)
      }
    val token =
      GSON.fromJson(response.body<String>(), IIkoAuthResponse::class.java).token
  ...
  suspend fun getGroups(branchId: Long?, merchantId: Long?): ResponseData {
    ...
    if (organization?.iikoOrganizationId == null)
      throw BadRequest("In this branch, the integration with the iiko system is not set up")
    val response =
      client.post("https://api-ru.iiko.services/api/1/nomenclature") {
        contentType(ContentType.Application.Json)
    ...
    return ResponseData(
      data = Gson().fromJson(response.body<String>(), IIkoGroupResponse::class.java)
    )
  }
  ...
  suspend fun getCategory(branchId: Long?, merchantId: Long?): ResponseData {
    val organization = MerchantIntegrateRepository.get(merchantId)
    if (organization?.iikoOrganizationId == null)
      throw BadRequest("in this branch didn't integration with iiko system")
    val response =
      client.post("https://api-ru.iiko.services/api/1/nomenclature") {
        contentType(ContentType.Application.Json)
        bearerAuth(authTokenMap[merchantId] ?: auth(merchantId ?: -1))
    ...
    return ResponseData(
      data = Gson().fromJson(response.body<String>(), IIkoCategoryResponse::class.java)
    )
  }
  ...
  private fun getProductsFromList(
    ...
    return products.products!!.filter { p ->
      p.productCategoryId == categoryId &&
        p.groupId == groupId &&
        p.sizePrices?.get(0)?.price?.isIncludedInMenu == true
    }
  }
  ...
  suspend fun getPayment(principal: BasePrincipal?): ResponseData {
    ...
    val response =
      client.post("https://api-ru.iiko.services/api/1/payment_types") {
        contentType(ContentType.Application.Json)
  ...
  suspend fun getTerminalGroup(branchId: Long?, merchantId: Long?): ResponseData {
    ...
    val response =
      client.post("https://api-ru.iiko.services/api/1/terminal_groups") {
        contentType(ContentType.Application.Json)
        bearerAuth(authTokenMap[merchantId] ?: auth(merchantId ?: -1))
        setBody(Gson().toJson(body))
      }
  ...
  suspend fun createOrder(merchantId: Long?, orderId: Long): ResponseModel {
    ...
    val response =
      client.post("https://api-ru.iiko.services/api/1/deliveries/create") {
  ...
  private suspend fun createOrderObj(orderId: Long): IIkoOrder {
    val order =
      OrderService.getById(orderId, "user", "products", "payment_type", "address", "branch")
    ...
    return IIkoOrder(
      ...
      createOrderSettings = CreateOrderSettings(transportToFrontTimeout = 8, checkStopList = true),
      order =
        IIkoOrderItem(
          ...
          orderServiceType =
            if (order?.serviceType == BaseEnums.DELIVERY) "DeliveryByCourier"
          ...
          guests = Guests(splitBetweenPersons = true, count = 1),
          customer =
            Customer(
          ...
          payments =
            arrayListOf(
              Payments(
                ...
                paymentTypeId =
                  PaymentTypeRepositoryImpl.getForIIko(order?.paymentMethod?.id).iikoId,
