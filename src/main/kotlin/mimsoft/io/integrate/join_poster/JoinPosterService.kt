...
object JoinPosterService {
  ...
  suspend fun sendOrder(order: Order?): ResponseModel {
    ...
    if (order?.merchant?.id == null)
      return ResponseModel(httpStatus = HttpStatusCode.BadRequest, body = "Merchant id is null")
    ...
    return if (response.status.value == 200) {
      ...
      ResponseModel(httpStatus = HttpStatusCode.BadRequest, body = response.body<String>())
    }
  }

  private suspend fun setPosterId(order: Order?) {
    val query =
      "update orders set post_id = ${order?.posterId} where id = ${order?.id} and not deleted"
