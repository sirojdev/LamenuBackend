...
object JowiService {
  ...
  suspend fun createOrder(order: Order) {
    ...
    val response =
      client.post("https://api.jowi.club/v3/orders") {
  ...
  suspend fun getRestaurant(): ArrayList<JowiRestaurant> {
    val response =
      client.get(
        "https://api.jowi.club/v3/restaurants?api_key=${JowiConst.API_KEY}&&sig=${JowiConst.sig}"
      ) {
        contentType(ContentType.Application.Json)
      }
  ...
  private suspend fun getRestaurantFromDB(): ArrayList<JowiRestaurant> {
    val query =
      """select * from $JOWI_RESTAURANT
        """
        .trimIndent()
    var list = ArrayList<JowiRestaurant>()
    withContext(Dispatchers.IO) {
      StaffService.repository.connection().use {
        var rs = it.prepareStatement(query).apply {}.executeQuery()
  ...
  suspend fun joinRestaurant(branchId: Long, restaurantId: String, merchantId: Long): Boolean {
    ...
    withContext(Dispatchers.IO) {
      StaffService.repository.connection().use {
        val st = it.prepareStatement(query).apply { setString(1, restaurantId) }.executeUpdate()
        rs = st == 1
      }
    }
    return rs
  ...
  suspend fun getCourseById(id: String): Courses? {
    val query = "select * from $JOWI_PRODUCTS where jowi_id = ? "
    var rs: Courses? = null
    withContext(Dispatchers.IO) {
      StaffService.repository.connection().use {
        val st = it.prepareStatement(query).apply { setString(1, id) }.executeQuery()
        if (st.next()) {
          rs = getOneCourse(st)
        }
      }
  ...
  suspend fun getProducts(restaurantId: String?): JowiCoursesResponse? {
    val response =
      client.get(
        "https://api.jowi.club/v3/restaurants/$restaurantId/courses?api_key=${JowiConst.API_KEY}&&sig=${JowiConst.sig}"
      ) {
        contentType(ContentType.Application.Json)
      }
