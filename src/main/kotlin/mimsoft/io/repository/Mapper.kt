...
object Mapper {
  ...
  inline fun <reified T : Any, reified R : Any> toTable(dto: T): R? {
    ...
    val tableValues =
      tableParameters.mapValues { (name, parameter) ->
    ...
    return tableConstructor.callBy(
      tableValues
        .filterKeys { it != null }
        .mapKeys { tableConstructor.parameters.find { p -> p.name == it.key }!! }
    ) as? R
  }
  ...
  inline fun <reified T : Any, reified R : Any> toDto(table: T?): R? {
    ...
    val dtoValues =
      dtoParameters.mapValues { (name, parameter) ->
        val propertyName = name?.replace("Uz", "")?.replace("Ru", "")?.replace("En", "")
        ...
        if (parameter.type.classifier == TextModel::class) {
          ...
          tableClass.memberProperties
            .filter { it.name.startsWith(propertyName ?: "") }
            .forEach { prop ->
    ...
    return dtoConstructor.callBy(
      dtoValues
        .filterKeys { it != null }
        .mapKeys { dtoConstructor.parameters.find { p -> p.name == it.key }!! }
    ) as R
  }

  fun getDefaultParameterValue(parameter: KParameter, clazz: KClass<*>): Any? {
    val defaultValueConstructor =
      clazz.primaryConstructor?.parameters?.firstOrNull { it == parameter }
