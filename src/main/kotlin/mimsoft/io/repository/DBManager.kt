...
object DBManager : BaseRepository {
  ...
  override suspend fun <T : Any> getPageData(
    ...
    if (where == null) {
      ...
      query =
        "SELECT $columns FROM $tName $whereClause  deleted = false $limitClause $offsetClause  "
    } else {
      val whereClause = generateWhereClause(where)
      query =
        "SELECT $columns FROM $tName $whereClause and deleted = false $limitClause $offsetClause  "
    }
  ...
  override suspend fun <T : Any> getJoinPageData(
    ...
    val query =
      "SELECT $columns FROM $tName $whereClause $limitClause $offsetClause and deleted = false"
  ...
  private fun generateWhereClause(where: List<Any>, whereName: List<String>): String {
    ...
    return "WHERE " +
      wherePairs.joinToString(" AND ") { (value, name) -> "$name = ${escapeSqlValue(value)}" }
  }
  ...
  override suspend fun getData(
    ...
    val tName =
      tableName
        ...
        ?: throw IllegalArgumentException("Table name must be provided")
    val columns = dataClass.memberProperties.joinToString(", ") { camelToSnakeCase(it.name) }

    val hasDeleted =
      if (dataClass.memberProperties.find { it.name == "deleted" } != null) "WHERE NOT deleted"
      else "WHERE true"
  ...
  override suspend fun <T : Any> postData(
    dataClass: KClass<T>,
    dataObject: T?,
    tableName: String?
  ): Long? {
    ...
    val filteredProperties =
      dataClass.memberProperties.filter {
        it.name != "deleted" && it.name != "updated" && it.name != "id"
      }
  ...
  override suspend fun <T : Any> updateData(
    ...
    val filteredProperties =
      dataClass.memberProperties.filter {
        it.name != "deleted" && it.name != "created" && it.name != "id"
      }
    ...
    withContext(Dispatchers.IO) {
      connection().use { connection ->
        ...
        val idValue =
          dataObject?.let { dataClass.memberProperties.first { it.name == idColumn }.get(it) }
        statement.setLong(filteredProperties.size + 1, idValue as Long)

        if (statement.executeUpdate() == 1) response = true
      }
    }
    return response
  }
        ...
        val columnType =
          when (property.type) {
            ...
            Boolean::class.createType(nullable = true) ->
              "BOOLEAN${if (columnName == "deleted") " DEFAULT FALSE" else ""}"
