...
fun Route.routeToWaitersInfo() {
  ...
  post("firebase") {
    ...
    call.respond(
      ResponseData(
        data =
          DeviceController.editFirebaseWithDeviceId(deviceId = session?.deviceId, token = firebase)
      )
    )
  }
  ...
  patch("update/image") {
    ...
    if (WaiterService.updateWaiterImageProfile(result, principal?.staffId ?: -1))
  ...
  patch("update/password") {
    ...
    if (dto.newPassword != dto.newConfirmPassword)
      throw BadRequest("new password and confirm password not equal")
    val staff =
      waiterService.getWithPasswordAndImages(principal?.staffId)
