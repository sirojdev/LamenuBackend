...
object WaiterTableRepository {
  ...
  suspend fun getActiveTablesWaiters(
    waiterId: Long?,
    limit: Int,
    offset: Int
  ): DataPage<WaiterTableDto> {
    val query =
    ...
    withContext(DBManager.databaseDispatcher) {
      repository.connection().use {
        val rs = it.prepareStatement(query).apply {}.executeQuery()
        while (rs.next()) {
  ...
  private fun getTables(rs: ResultSet): WaiterTableDto {
    return WaiterTableDto(
      ...
      table =
        TableDto(name = rs.getString("t_name"), room = RoomDto(name = rs.getString("r_name"))),
      count = rs.getInt("count")
    )
  }

  suspend fun getFinishedTablesWaiters(
    waiterId: Long?,
    limit: Int,
    offset: Int
  ): DataPage<WaiterTableDto> {
    val query =
      """select count(*) over () as count, wt.*, t.name t_name, r.name r_name
  ...
  suspend fun joinToWaiter(waiterId: Long?, branchId: Long?, order: WaiterNewOrderDto): Boolean {
    ...
    withContext(Dispatchers.IO) {
      repository.connection().use {
        val rs = it.prepareStatement(query).apply {}.executeUpdate()
        result = rs == 1
      }
    }
    return result
  ...
  suspend fun joinToWaiter(waiterId: Long?, tableId: Long?, branchId: Long?): Boolean {
    val query =
      "INSERT INTO waiter_table (waiter_id, table_id, join_time)\n" +
