...
fun Route.routeToWaitersTables() {
    ...
    route("table") {
        ...
        get("tables") {
            ...
            val tables = TableService.getTablesWaiter(roomId = roomId, branchId = branchId, merchantId = merchantId)
            if (tables.isEmpty()) {
                call.respond(HttpStatusCode.NotFound)
                return@get
            }
        ...
        post("create/book") {
            ...
            val response = BookRepositoryImpl.add(
                bookDto = bookDto.copy(
        ...
        patch("update/book") {
            val pr = getPrincipal()
            val bookDto = call.receive<BookDto>()
            val response = BookRepositoryImpl.update(
                bookDto = bookDto.copy(
                    merchantId = pr?.merchantId,
                    status = BookStatus.ACCEPTED
                )
