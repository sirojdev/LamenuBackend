...
object WaiterBookService {
  ...
  suspend fun add(book: WaiterBookDto, principal: BasePrincipal?): WaiterBookDto {
    if (book.time!! < Timestamp(System.currentTimeMillis()))
      throw BadRequest("time must be greater than current time")
    val user = UserRepositoryImpl.get(book.client?.phone, principal?.merchantId)
    val clientId =
      if (user == null)
        book.client?.phone?.let {
          UserRepositoryImpl.addNewClientFromWaiter(it, principal?.merchantId ?: -1)
        }
      else user.id
    val query =
      """insert into book (merchant_id,client_id,table_id,time,created,
                ...
                ?,?,${book.visitorCount},${principal?.branchId})"""
        .trimMargin()
    ...
    return if (user != null) book.copy(client = user)
    else book.copy(client = book.client?.copy(id = clientId))
  }
}
