...
object CourierOrderService {
  ...
  suspend fun joinOrderToCourier(courierId: Long?, orderId: Long?): Order? {
    val query =
      " update orders set courier_id = $courierId , on_wave  = ? " +
  ...
  suspend fun updateStatus(courierId: Long?, orderId: Long?, status: OrderStatus?): Int {
    ...
    if (status == OrderStatus.DELIVERED) {
      ...
      query +=
        " where courier_id = $courierId and id = $orderId and deleted = false and type = 'DELIVERY' "
    ...
    val result =
      withContext(Dispatchers.IO) {
        repository.connection().use {
          it.prepareStatement(query).apply { setString(1, status?.name) }.executeUpdate()
        }
      }
    return result
  }
    ...
    val result =
      withContext(Dispatchers.IO) {
        repository.connection().use {
          it
            .prepareStatement(query)
            .apply { setString(1, OrderStatus.ACCEPTED.name) }
            .executeUpdate()
        }
      }
    if (result == 1) {
      //            return
      // ResponseModel(body=OrderService.getById(orderId,"user","branch","payment_type")?:"Not
      // found", httpStatus = HttpStatusCode.OK)
