...
object BoardSocketService {
  ...
  fun setBoardConnection(
    ...
    val connection =
      boardConnections.find {
        it.boardId == boardId && it.branchId == branchId && it.merchantId == merchantId
      }
    if (connection == null) {
      boardConnections +=
        BoardConnection(
  ...
  suspend fun sendOrderToBoard(order: Order, type: BoardOrderStatus, action: Action) {
    ...
    val connection =
      boardConnections.find {
        it.boardId == dto?.id && it.branchId == branchId && it.merchantId == merchantId
      }
    connection
      ?.session
      ?.send(Gson().toJson(SocketOrderResponseDto(order = order, type = type, action = action)))
  }
}
