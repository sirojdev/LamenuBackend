...
object BoardAuthService {
  ...
  suspend fun singUp(boardAuthDto: BoardAuthDto): ResponseModel {
    if (
      boardAuthDto.merchantId == null ||
        boardAuthDto.branchId == null ||
        boardAuthDto.username?.length!! < 6 ||
        boardAuthDto.password?.length!! < 6
    ) {
      ...
      repository.connection().use {
        ...
        response =
          if (rs.next()) {
            ...
            ResponseModel(body = " username or password incorrect", HttpStatusCode.BadRequest)
          }
      }
    }
    return response
