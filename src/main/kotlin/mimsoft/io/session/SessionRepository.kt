...
object SessionRepository {
  ...
  private suspend fun expireOtherSession(deviceId: Long?, merchantId: Long?): Boolean {
    val query =
      "update session set is_expired = true, updated = ? where device_id = $deviceId " +
  ...
  suspend fun get(phone: String?, deviceId: Long?): SessionTable? {
    return DBManager.getPageData(
        ...
        where = mapOf("phone" to phone as Any, "device_id" to deviceId as Any)
      )
      ?.data
      ?.firstOrNull()
  }
  ...
  suspend fun expire(uuid: String?): Boolean =
  ...
  suspend fun getUserSession(sessionUuid: String): SessionTable? {
    val query = "select * from session where " + "uuid = ? and user_id is not null"
  ...
  suspend fun getStaffSession(sessionUuid: String): SessionTable? {
    val query = "select * from session where " + "uuid = ? and stuff_id is not null"
  ...
  suspend fun expireOthers(uuid: String?, userId: Long?, merchantId: Long?): Boolean {
    val query =
      "update session set is_expired = true, updated = ? where " +
  ...
  suspend fun getStuffDevices(
    stuffId: Long?,
    merchantId: Long?,
    uuid: String?
  ): List<DeviceModel?> {
    val query =
      "select d.* , s.uuid s_uuid  from session s left join device d on d.id = s.device_id " +
