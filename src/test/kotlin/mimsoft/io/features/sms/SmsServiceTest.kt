...
class SmsServiceTest {
  ...
  fun getAll() = testApplication { // TODO: there is an error...
    ...
    println("rs: $response")
    if (response?.data?.isEmpty() == true) assertNotNull(response.data)
  }
  ...
  fun post() = testApplication {
    val messageDto = MessageDto(id = 6)
    val status = Status.SENT
    val smsDto = SmsDto(clientCount = 32, message = messageDto, status = status, merchantId = 1)
  ...
  fun update() = testApplication {
    ...
    val status = Status.SENT
    val smsDto =
      SmsDto(id = 5, clientCount = 32, message = messageDto, status = status, merchantId = 7)
