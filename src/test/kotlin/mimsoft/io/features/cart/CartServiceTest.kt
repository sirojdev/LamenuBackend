...
class CartServiceTest { // TODO: Xatolik bor...
  ...
  fun check() = testApplication {
    ...
    val userDto = UserDto(id = 24)
    val staffDto = StaffDto(id = 9)
    val merchantDto = MerchantDto(id = 1)
    val branchDto = BranchDto(id = 31)
    val cartItem = CartItem()

    val paymentDto = PaymentDto(id = 1)
  ...
  fun productCount() = testApplication {
    val optionDto = OptionDto(id = 34)
    val productDto = ProductDto(id = 12)
    val extraDtoList = listOf(ExtraDto(id = 45))
    val cartItemList =
      listOf(
        CartItem(
          option = optionDto,
